...
@section('content')
  ...
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h1 class="h2 fw-extrabold mb-1 text-main">Edit Dokumentasi</h1>
      <p class="text-muted-custom mb-0 fs-6">Perbarui data atau lampiran media dokumentasi kegiatan.</p>
    </div>
  </div>
    ...
    <form action="{{ route('dokumentasi.update', $dokumentasi->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row g-4">
        ...
        <div class="col-12" id="input-gambar-container">
          <label for="file_path" class="form-label fw-bold text-main">Ganti Gambar Dokumentasi</label>
          ...
          @if($dokumentasi->file_path && $dokumentasi->tipe == 'gambar')
            <div class="mb-3">
              ...
              <img src="{{ asset('storage/' . $dokumentasi->file_path) }}"
                   ...
                   style="max-height: 180px; object-fit: contain; background: var(--card-sub-bg); padding: 8px;">
            </div>
          @endif
          ...
          <small class="text-muted-custom mt-1 d-block fs-8">Abaikan jika tidak ingin mengubah gambar saat ini. Format: JPG, PNG, WEBP, GIF. Maksimal 5 MB.</small>
          @error('file_path')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
          ...
          <div id="imagePreviewContainer" class="mt-3 d-none">
            ...
            <div>
              <img id="imagePreview" src="#" alt="Pratinjau Gambar Baru" class="rounded-4 border-0 shadow-sm" style="max-height: 220px; object-fit: contain; background: var(--card-sub-bg); padding: 8px;">
            </div>
          </div>
        </div>
      ...
      <div class="d-flex flex-wrap gap-2 justify-content-end pt-4 mt-4" style="border-top: 1px solid var(--card-sub-bg);">
        ...
        <button type="submit" class="btn btn-dark-pill px-4 py-2 fs-7 d-flex align-items-center gap-2">
          <i class="bi bi-save"></i> Perbarui Dokumentasi
        </button>
      </div>
    </form>
  </div>
