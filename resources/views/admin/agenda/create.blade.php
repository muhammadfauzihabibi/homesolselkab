...
@section('content')
  ...
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h1 class="h2 fw-extrabold mb-1 text-main">Tambah Agenda Baru</h1>
      <p class="text-muted-custom mb-0 fs-6">Buat jadwal kegiatan atau acara resmi baru untuk Pemda Solok Selatan.</p>
    </div>
  </div>
    ...
    <form action="{{ route('agenda.store') }}" method="POST">
      ...
      <div class="mb-4">
        ...
        <textarea name="description"
                  ...
                  style="background: var(--card-sub-bg); color: var(--text-dark); border-radius: 16px;"
                  placeholder="Tuliskan detail singkat tentang agenda ini (opsional)...">{{ old('description') }}</textarea>
        @error('description')
          <div class="invalid-feedback">{{ $message }}</div>
        @enderror
      </div>
      ...
      <div class="d-flex flex-wrap gap-2 justify-content-end pt-3" style="border-top: 1px solid var(--card-sub-bg);">
        ...
        <button type="submit" class="btn btn-dark-pill px-4 py-2 fs-7">Simpan Agenda</button>
      </div>
  ...
  <script>
    // Auto-set end_date min to start_date value
