...
@section('content')
  ...
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h1 class="h2 fw-extrabold mb-1 text-main">Edit Halaman</h1>
      <p class="text-muted-custom mb-0 fs-6">Formulir pembaruan data halaman statis portal daerah.</p>
    </div>
  </div>
    ...
    <form action="{{ route('page.update', $page->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row g-4">
        ...
        <div class="col-12">
          ...
          <small class="text-muted-custom mt-1 d-block fs-8">Biarkan kosong jika tidak ingin mengubah thumbnail. Format: JPG, PNG, WEBP. Maksimal 5 MB.</small>
          @error('thumbnail')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror

          <!-- Preview Image Container (Thumbnail Eksisting / Baru) -->
          <div id="imagePreviewContainer" class="mt-3 {{ $page->thumbnail ? '' : 'd-none' }}">
            ...
            <div>
              <img id="imagePreview" 
                   ...
                   style="max-height: 200px; object-fit: contain; background: var(--card-sub-bg); padding: 8px;">
            </div>
          </div>
        </div>
  ...
  <script>
    ...
    // Simpan URL Thumbnail Asli
    const originalThumbnail = "{{ $page->thumbnail ? asset('storage/' . $page->thumbnail) : '' }}";
    ...
    // Reset Image Preview ke Gambar Asli
