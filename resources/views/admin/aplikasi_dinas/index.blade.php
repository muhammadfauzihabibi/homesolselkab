...
@section('content')
  ...
  <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
    <div>
      <h1 class="h2 fw-extrabold mb-1 text-main">Manajemen Aplikasi Dinas</h1>
      <p class="text-muted-custom mb-0 fs-6">Kelola daftar dan link aplikasi internal/eksternal milik Dinas Pemda Solok Selatan.</p>
    </div>
    <div>
      <a href="{{ route('aplikasi-dinas.create') }}" class="btn btn-dark-pill d-flex align-items-center gap-2">
        <i class="bi bi-plus-lg"></i> Tambah Aplikasi Baru
      </a>
    </div>
  </div>
  ...
  <div class="glass-card p-3 mb-4 shadow-sm">
    <form action="{{ route('aplikasi-dinas.index') }}" method="GET" class="row g-3 align-items-center">
      <div class="col-12 col-md-6">
        <div class="input-group">
          ...
          <input type="text" name="search" class="form-control border-0 py-2 fs-7" style="background: var(--card-sub-bg); color: var(--text-dark); border-top-right-radius: 999px; border-bottom-right-radius: 999px;" placeholder="Cari nama aplikasi atau URL..." value="{{ request('search') }}">
        </div>
      </div>
  ...
  <!-- Aplikasi Dinas Data Table Card -->
  <div class="glass-card p-3 shadow-sm">
      ...
      <table class="table align-middle border-0 mb-0 glass-table">
        <thead>
          <tr class="text-muted-custom fs-8 text-uppercase">
            ...
            <th class="border-0">Nama Aplikasi</th>
            <th class="border-0">URL Aplikasi</th>
        ...
        <tbody class="fs-7 fw-semibold text-main">
          @forelse($aplikasiDinasList as $index => $aplikasi)
            <tr>
              <td class="text-muted-custom">{{ $aplikasiDinasList->firstItem() + $index }}</td>
              <td>
                <div class="d-flex align-items-center gap-3">
                  ...
                  <div>
                    <span class="fw-bold fs-6 d-block text-main">{{ $aplikasi->nama }}</span>
                    <small class="text-muted-custom">Aplikasi Dinas Solsel</small>
                  </div>
                </div>
              </td>
              <td>
                <a href="{{ $aplikasi->url }}" target="_blank" class="text-decoration-none fw-semibold" style="color: #38bdf8;">
              ...
              <td class="text-end">
                <div class="d-flex justify-content-end gap-2">
                  ...
                  <form action="{{ route('aplikasi-dinas.destroy', ['aplikasiDinas' => $aplikasi->id]) }}" method="POST" class="delete-form" style="display:inline;">
                    @csrf
                    @method('DELETE')
    ...
    @if($aplikasiDinasList->hasPages())
      <div class="d-flex justify-content-between align-items-center pt-3 mt-3" style="border-top: 1px solid var(--card-sub-bg);">
        <small class="text-muted-custom">Menampilkan {{ $aplikasiDinasList->firstItem() }} - {{ $aplikasiDinasList->lastItem() }} dari {{ $aplikasiDinasList->total() }} Aplikasi Dinas</small>
