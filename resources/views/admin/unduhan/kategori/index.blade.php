...
@section('title', 'Kelola Kategori Unduhan')
...
@section('content')
  ...
  <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
    <div>
      <div class="d-flex align-items-center gap-2 mb-1">
        <h1 class="admin-page-title mb-0">Kelola Kategori Unduhan</h1>
        <span class="badge badge-solsel fs-8">Kategori Unduhan</span>
      </div>
      <p class="admin-page-subtitle">Kelola kategori dokumen untuk sistem Download Center.</p>
    </div>
    <div>
      <a href="{{ route('admin.unduhan.kategori.create') }}" class="btn btn-primary d-flex align-items-center gap-2">
  ...
  <div class="glass-card p-3 mb-4">
    <form action="{{ route('admin.unduhan.kategori.index') }}" method="GET" class="row g-2 align-items-center">
      ...
      <div class="col-12 col-md-3 d-flex gap-2">
        ...
        @if(request('search'))
          <a href="{{ route('admin.unduhan.kategori.index') }}" class="btn btn-glass-icon d-flex align-items-center justify-content-center flex-shrink-0" title="Reset Pencarian">
  ...
  <div class="glass-card p-3">
    ...
    <div class="d-flex justify-content-between align-items-center pb-3 mb-2 border-bottom">
      <h6 class="fw-bold  mb-0">Daftar Kategori Unduhan</h6>
            ...
            <tr>
              ...
              <td>
                <div class="d-flex flex-column">
                  ...
                  @if($kategori->deskripsi)
                    <small class="text-muted fs-8">{{ Str::limit($kategori->deskripsi, 60) }}</small>
                  @endif
                </div>
              </td>
              <td class="text-center">
                @if($kategori->icon)
                  <i class="{{ $kategori->icon }} fs-4 text-primary"></i>
                @else
                  <span class="text-muted">-</span>
                @endif
              </td>
              <td class="text-center">
                <span class="badge bg-info-subtle text-info rounded-pill px-2.5 py-1 fs-8 fw-bold">{{ $kategori->jenis_unduhans_count }}</span>
              </td>
              <td class="text-center">
                <span class="badge bg-success-subtle text-success rounded-pill px-2.5 py-1 fs-8 fw-bold">{{ $kategori->unduhans_count }}</span>
              </td>
              <td class="text-center">
              ...
              <td class="text-end">
                <div class="d-flex justify-content-end gap-1">
                  <a href="{{ route('admin.unduhan.kategori.edit', $kategori) }}" class="btn btn-glass-icon d-flex align-items-center justify-content-center" title="Edit Kategori">
            ...
            <tr>
              <td colspan="8" class="py-5 text-center text-muted">
                <div class="d-flex flex-column align-items-center gap-2">
                  ...
                  <span>Belum ada kategori unduhan ditemukan.</span>
                  <a href="{{ route('admin.unduhan.kategori.create') }}" class="btn btn-primary btn-sm rounded-pill px-3 py-1.5 fs-7 mt-1">
...
function confirmDelete(id, name) {
  Swal.fire({
    ...
    html: `Apakah Anda yakin ingin menghapus kategori <strong>"${name}"</strong>?<br><small class="text-muted">Pastikan kategori tidak memiliki jenis dokumen atau dokumen aktif.</small>`,
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      ...
      form.action = `{{ route('admin.unduhan.kategori.index') }}/${id}`;
      form.submit();
    }
  });
}
