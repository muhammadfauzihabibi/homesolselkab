...
@section('title', 'Edit Dokumen Unduhan')

@section('content')
  <!-- Header Page Title -->
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h1 class="admin-page-title">Edit Dokumen Unduhan</h1>
      <p class="admin-page-subtitle mb-0">
        Perbarui informasi dokumen "{{ $unduhan->judul }}".
  ...
  <div class="row g-4">
    <div class="col-lg-8">
      <form action="{{ route('unduhan.update', $unduhan) }}" method="POST" class="glass-card p-4">
        @csrf
        @method('PUT')
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <select class="form-select @error('kategori_unduhan_id') is-invalid @enderror" id="kategori_unduhan_id" name="kategori_unduhan_id" required>
              ...
              @foreach($kategoris as $kategori)
                <option value="{{ $kategori->id }}" {{ old('kategori_unduhan_id', $unduhan->kategori_unduhan_id) == $kategori->id ? 'selected' : '' }}>
                  {{ $kategori->nama }}
                </option>
              @endforeach
            </select>
          ...
          <div class="col-md-6">
            ...
            <select class="form-select @error('jenis_unduhan_id') is-invalid @enderror" id="jenis_unduhan_id" name="jenis_unduhan_id" required>
              ...
              @foreach($jenises as $jenis)
                <option value="{{ $jenis->id }}" {{ old('jenis_unduhan_id', $unduhan->jenis_unduhan_id) == $jenis->id ? 'selected' : '' }}>
                  {{ $jenis->nama }}
                </option>
              @endforeach
            </select>
          ...
          <div class="col-12">
            ...
            <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul" name="judul" value="{{ old('judul', $unduhan->judul) }}" placeholder="Contoh: APBD Kabupaten Solok Selatan Tahun 2025" required autofocus>
            @error('judul')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <small class="form-text text-muted mt-1 d-block fs-8">Slug URL: <code>{{ $unduhan->slug }}</code></small>
          </div>

          <!-- Tahun -->
          <div class="col-md-6">
            <label for="tahun" class="form-label fw-bold ">Tahun</label>
            <input type="number" class="form-control @error('tahun') is-invalid @enderror" id="tahun" name="tahun" value="{{ old('tahun', $unduhan->tahun) }}" min="1900" max="{{ date('Y') + 1 }}" placeholder="{{ date('Y') }}">
            @error('tahun')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="col-md-6">
            ...
            <input type="date" class="form-control @error('tanggal_publikasi') is-invalid @enderror" id="tanggal_publikasi" name="tanggal_publikasi" value="{{ old('tanggal_publikasi', $unduhan->tanggal_publikasi ? $unduhan->tanggal_publikasi->format('Y-m-d') : '') }}">
          ...
          <div class="col-12">
            ...
            <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="3" placeholder="Deskripsi dokumen...">{{ old('deskripsi', $unduhan->deskripsi) }}</textarea>
            @error('deskripsi')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="col-12">
            ...
            <div class="input-group">
              ...
              <input type="url" class="form-control border-0 py-2 fs-7 @error('google_drive_url') is-invalid @enderror" id="google_drive_url" name="google_drive_url" value="{{ old('google_drive_url', $unduhan->google_drive_url) }}" placeholder="https://drive.google.com/file/d/..." required>
            </div>
            @error('google_drive_url')
              <div class="invalid-feedback d-block">{{ $message }}</div>
            @enderror
            @if($unduhan->google_drive_url)
              <small class="form-text text-success mt-1 d-block fs-8">
                <i class="bi bi-check-circle me-1"></i>
                <a href="{{ $unduhan->google_drive_url }}" target="_blank" class="text-success fw-bold">Lihat file di Google Drive</a>
              </small>
            @endif
          </div>
          ...
          <div class="col-12">
            <div class="p-3 rounded-4 mb-2" style="background: var(--card-sub-bg);">
              <div class="form-check form-switch d-flex align-items-center gap-2 ps-0">
                <input class="form-check-input ms-0" type="checkbox" name="aktif" value="1" id="aktifSwitch" style="width: 2.8em; height: 1.5em;" {{ old('aktif', $unduhan->aktif) ? 'checked' : '' }}>
                <label class="form-check-label fw-bold  fs-7 ms-2" for="aktifSwitch">
                  Status Dokumen: <span class="status-text text-primary">{{ old('aktif', $unduhan->aktif) ? 'Aktif' : 'Nonaktif' }}</span>
                </label>
              </div>
        ...
        <div class="d-flex flex-wrap gap-2 justify-content-end pt-3 mt-3" style="border-top: 1px solid var(--card-sub-bg);">
          ...
          <button type="submit" class="btn btn-primary px-4 py-2 fs-7">Perbarui Dokumen</button>
        </div>
      </form>
    </div>
    ...
    <div class="col-lg-4">
      <div class="glass-card p-4 mb-3">
        <h6 class="fw-bold  mb-3 pb-2 border-bottom">
          <i class="bi bi-bar-chart-line me-2 text-primary"></i>Statistik Dokumen
        </h6>

        <div class="text-center mb-3">
          <div class="p-3 rounded-4" style="background: var(--card-sub-bg);">
            <div class="fs-2 fw-bold text-success">{{ number_format($unduhan->jumlah_unduhan ?? 0) }}</div>
            <div class="fs-8 text-muted">Total Unduhan Publik</div>
          </div>
        </div>
        ...
        <div class="mt-3 pt-3 border-top fs-8 text-muted">
          <div class="mb-1">
            <i class="bi bi-calendar3 me-1"></i> Dibuat: {{ $unduhan->created_at->format('d M Y, H:i') }}
          </div>
          @if($unduhan->updated_at != $unduhan->created_at)
            <div>
              <i class="bi bi-pencil me-1"></i> Diperbarui: {{ $unduhan->updated_at->format('d M Y, H:i') }}
            </div>
          @endif
        </div>
      </div>
      ...
      <div class="glass-card p-4">
        <h6 class="fw-bold  mb-3 pb-2 border-bottom">
          <i class="bi bi-link-45deg me-2 text-primary"></i>Link Akses Publik
        </h6>

        <div class="mb-3">
          <label class="fs-8 fw-bold  mb-1 d-block">Link Download Frontend:</label>
          <div class="input-group input-group-sm">
            <input type="text" class="form-control border-0 fs-8" style="background: var(--card-sub-bg);" value="{{ route('frontend.unduhan.download', $unduhan->slug) }}" readonly>
            <button class="btn btn-primary" type="button" onclick="copyToClipboard('{{ route('frontend.unduhan.download', $unduhan->slug) }}')">
        ...
        @if($unduhan->google_drive_url)
          <div>
            <a href="{{ $unduhan->google_drive_url }}" target="_blank" class="btn btn-outline-success btn-sm w-100 rounded-pill py-2 fs-8 fw-bold">
              <i class="bi bi-cloud-arrow-up me-1"></i> Buka Google Drive
            </a>
          </div>
        @endif
      </div>
    ...
    <div class="modal-content" style="background: var(--card-bg, #1a1e29); border: 1px solid var(--card-border, rgba(255,255,255,0.08)); border-radius: 24px;">
      ...
      <div class="modal-body p-4">
          ...
          <div class="p-3 rounded-3" style="background: var(--card-sub-bg, rgba(255,255,255,0.03));">
            <ul class="list-unstyled mb-0 fs-7 text-muted">
              <li class="mb-2 d-flex align-items-start gap-2">
                ...
                <span>Pastikan file PDF sudah terunggah ke Google Drive dinas/pemerintah.</span>
              </li>
              <li class="mb-2 d-flex align-items-start gap-2">
                <i class="bi bi-check2 text-success mt-1"></i>
        ...
        <div class="mb-4">
          ...
          <small class="text-muted fs-8 d-block mt-1">Sistem secara otomatis mengubah tautan ini menjadi link unduhan langsung yang dapat diakses oleh masyarakat.</small>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  kategoriSelect.addEventListener('change', function() {
    ...
    const currentJenisId = '{{ old("jenis_unduhan_id", $unduhan->jenis_unduhan_id) }}';
