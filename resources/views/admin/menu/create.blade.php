...
@section('content')
  ...
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h1 class="admin-page-title">Tambah Menu</h1>
      <p class="admin-page-subtitle">Form penambahan data menu portal daerah.</p>
    </div>
  </div>
  ...
  <div class="glass-card p-4 w-100">
    <form action="{{ route('menu.store') }}" method="POST">
        ...
        <div class="col-md-6">
          ...
          <div class="input-group">
            ...
            <select class="form-select border-0 py-2 fs-7 fw-semibold @error('tipe') is-invalid @enderror"
                    ...
                    name="tipe"
                    required>
              <option value="internal" {{ old('tipe', 'internal') == 'internal' ? 'selected' : '' }}>Halaman Internal (Dropdown / Parent)</option>
              <option value="external" {{ old('tipe') == 'external' ? 'selected' : '' }}>Link Eksternal</option>
            </select>
          </div>
          @error('tipe')
            <div class="invalid-feedback">{{ $message }}</div>
      ...
      <div class="d-flex flex-wrap gap-2 justify-content-end pt-3" style="border-top: 1px solid var(--card-sub-bg);">
        ...
        <button type="submit" class="btn btn-primary px-4 py-2 fs-7">Simpan Menu</button>
      </div>
    </form>
  </div>
