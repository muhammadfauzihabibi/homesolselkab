...
@section('content')
  ...
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h1 class="h2 fw-extrabold mb-1 text-main">Tambah OPD Baru</h1>
      <p class="text-muted-custom mb-0 fs-6">Daftarkan Organisasi Perangkat Daerah (OPD) atau subdomain layanan publik baru.</p>
    </div>
  </div>
  ...
  <div class="glass-card p-4 shadow-sm w-100">
    <form action="{{ route('opd.store') }}" method="POST">
      ...
      <div class="row g-3 mb-3">
        ...
        <div class="col-md-6">
          ...
          <select name="kategori" id="kategori" class="form-select border-0 py-2.5 px-3 fs-7 fw-semibold @error('kategori') is-invalid @enderror" style="background: var(--card-sub-bg); color: var(--text-dark); border-radius: 16px;" required>
            ...
            <option value="Badan" {{ old('kategori') == 'Badan' ? 'selected' : '' }}>Badan</option>
        ...
        <div class="col-md-6">
          ...
          <input type="url" 
                 ...
                 style="background: var(--card-sub-bg); color: var(--text-dark); border-radius: 16px;"
                 placeholder="https://diskominfo.solokselatankab.go.id" 
                 value="{{ old('url') }}" 
                 required>
          @error('url')
            <div class="invalid-feedback">{{ $message }}</div>
      ...
      <div class="d-flex flex-wrap gap-2 justify-content-end pt-2" style="border-top: 1px solid var(--card-sub-bg);">
        ...
        <button type="submit" class="btn btn-dark-pill px-4 py-2 fs-7">Simpan Data OPD</button>
      </div>
